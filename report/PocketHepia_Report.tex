...
{
    name={CRUD},
    description={Create-Read-Update-Delete (CRUD) are the four main methods used to handle data stored in a persistent storage, such as a database},
...
{
    name={PWA},
    description={Progressive Web App (PWA) are web applications that work on desktop and mobile with a responsive layout, provide offline functionality and can often be installed like "native" applications},
    first={Progressive Web App (PWA)},
...
{
    name={RTD},
    description={Record Type Definition (RTD) are used to specify the format and rules for building standard record types used in the NDEF format},
    first={Record Type Definition (RTD)},
    long={Record Type Definition}
}
...
\newmintedfile[dockercode]{docker}{
breaklines,
bgcolor=mintedbackground,
linenos=true,
numberblanklines=true,
...
%%and for showing me during all the years I have known him how not to handle project deadlines so that I didn't risk making the same mistakes.
...
\begin{code}
\yamlcode{assets/code/conf/compose.yml}
...
\begin{code}
	\nginxcode{assets/code/conf/nginx.conf}
...
\begin{code}
	\jscode{assets/code/js/routers.js}
...
\begin{code}
\dockercode{assets/code/conf/backend.conf}
...
\begin{code}
\jscode{assets/code/js/interceptor.js}
	\caption{The HTTP Interceptor Service that inserts the JWT in the Request}
\end{code}
\subsection{Routing}
\begin{code}
	\tscode{assets/code/typescript/implementedRoutes.ts}
...
\begin{code}
\dockercode{assets/code/conf/frontend.conf}
...
\begin{code}
	\kotlincode{assets/code/kotlin/dataTransformers.kt}
...
%%TODO add ref for recyclerview, adapters etc
...
\begin{figure}[H]
\begin{center}
	\includegraphics[width=\textwidth]{assets/screens/android/navigation}
...
\begin{figure}[H]
\begin{center}
	\includegraphics[width=\textwidth]{assets/screens/android/transactions_payment}
...
\begin{figure}[H]
\begin{center}
	\includegraphics[width=\textwidth]{assets/screens/android/user_detail_nfc}
...
\dirtree{%
.1 ITI\_IN\_diplome\_Lovino\_Hoerdt\_2018.
.2 ITI\_IN\_memoire\_diplome\_Lovino\_Hoerdt\_2018.pdf.
.2 ITI\_IN\_resume\_diplome\_Lovino\_Hoerdt\_2018.pdf.
.2 ITI\_IN\_src\_diplome\_Lovino\_Hoerdt\_2018.
.3 android.
.4 PocketHepia/\DTcomment{Contains the Android Studio project}.
.3 backend.
.4 Dockerfile.
.4 certs/.
.4 pocketHepiaServer/\DTcomment{Contains the NPM backend project}.
.3 conf.
.4 proxy.conf.json\DTcomment{Configuration file to run Angular locally}.
.3 db.
.4 Dockerfile.
.4 data/\DTcomment{Contains the database attached volume}.
.3 docker-compose.yml.
.3 docs.
.4 android/\DTcomment{Contains the Android HTML Documentation}.
.4 api/\DTcomment{Contains the APIDoc HTML Documentation}.
.3 frontend.
.4 Dockerfile.
.4 certs/.
.4 pockethepia/\DTcomment{Contains the NPM Angular project}.
.4 pockethepia.conf.
}
...
%%TODO write deployment guide, two parts: pull images and run with docker compose, build images with sources and run them locally
%%TODO add the fact that you need to direct pockethepia.maximelovino.ch domain to the IP of the machine used
%%TODO talk about calling init etc
