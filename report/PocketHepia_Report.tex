...
The endpoints of the \gls{api} are separated in categories, each with their own Router and Controller for the categories. We did this in order to avoid cramming every Route in a single Router and manage distinctively each category of Routes. All Routers can be found in the \verb+routes+ directory with the \verb+index+ Router handling the root Router and linking all sub-Routers. \\
