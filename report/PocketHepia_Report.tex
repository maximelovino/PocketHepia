...
%%TODO add caption and \begin{code} to each code listing
%%TODO replace \verb by \mintinline when possible

%%TODO convert this to english
%%TODO add RFID, add JWT, add SDK, add JS, add JSON, add BSON, add API, add PWA, add REST, add EPFL, add RTD, add ASCII, add UTF, add GDPR, add OS, add CLI, add NPM, add SQL, add TS
...
\begin{document}
%%TODO change titlepage to ressemble in text and form the official titlepage of other documents
\begin{titlepage}
	\centering
...
\listoflistings
\listoftables
%%TODO add typographic conventions
...
You have to setup \emph{Foreground Dispatch}\cite{android:doc:nfc_foregroundDispatch} at the Activity level, preferably in the \mintinline[breaklines]{kotlin}{fun onCreate(savedState: Bundle?)} method and then enable it and disable on resume and pause of the Activity respectively. If you need to use it in multiple activities, it is easier to setup a superclass Activity and extends all other Activities from it. We can then just override the \mintinline[breaklines]{kotlin}{fun onNewIntent(intent: Intent)} in the specific Activity for which we need to handle NFC specifically. In the superclass Activity, we can just discard the received NFC intent.\\
...
Now, if we want to read a NFC Tag inside an Activity, we just need to override the \mintinline[breaklines]{kotlin}{fun onNewIntent(intent: Intent)} method. For example, when assigning a Tag to a new \verb+User+, we have to get the NFC Tag unique identifier and send it to our backend. To do that, we can use the following code:
\begin{code}
...
First of all, we can generate the Guard that we're going to call the \verb+AuthGuard+ using the Angular CLI, we have to add it to the \verb+providers+ array in the AppModule as well:
\begin{code}
	\begin{shell}
ng generate guard auth
	\end{shell}
	\caption{Generating a guard using Angular CLI}
\end{code}
...
MongoDB is a document-oriented database\cite{wiki:document_oriented_db} launched in 2007 that store JSON-like documents in collections. It is currently one of the most used NoSQL data stores. The document would be the equivalent to a row in SQL and the collection is the equivalent to a table.
...
Node.js is often used to build backend REST APIs using the open source Express\cite{express:website}\cite{github:express} web framework that enables the creation of a simple web server to handle HTTP requests.
...
\subsection{Express}
Express is a simple web framework that is often used to easily write REST APIs Backends. It can integrate with PassportJS (see \ref{passport}) and other Express middlewares. \\
...
The documentation of the Backend API, generated with APIDoc is available as an interactive version on \url{https://maximelovino.github.io/PocketHepia/api/}
