...
%%TODO add RFID, add JWT, add SDK, add JS, add JSON, add BSON, add API, add PWA, add REST, add EPFL, add RTD, add ASCII, add UTF, add GDPR, add OS, add CLI, add NPM
...
\paragraph{Templates}
Templates are just HTML files on the surface with additional features inherent to Angular. One of these is the property binding. If you have a public property in the TypeScript file, you can access it and display it in the Template by using curly braces.
...
The parent Component can have properties for each child Component by selecting it as a property using the \verb+@ViewChild()+ annotation. As an example, we can take a \verb+UsersComponent+ which integrate a \verb+UserCreationComponent+ and a \verb+UserTableComponent+. The goal here is to refresh the table when a new User is created. To accomplish this, the \verb+UserCreationComponent+ must emit an event when a new User is created and the parent Component will catch it and call a method on the table to refresh it. \\
...
%%TODO talk about parametric components with @Input()
\subsubsection{Services}
\paragraph{Dependency injection}
%%TODO show passing a service as a parameter to a component
%%TODo part of service perhaps
\subsubsection{Routing and Guards}
\subsection{RxJS - Observables}
\subsection{Reactive Forms}
\subsection{TypeScript}
%%TODO \cite{github:typescript}
\subsection{Angular Material}
%%TODO \cite{angular:material:website}
%%TODO \cite{github:angular:material}
\section{Containers}
